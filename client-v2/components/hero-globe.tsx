"use client";

import { useEffect, useRef } from "react";
import createGlobe from "cobe";

export default function HeroGlobe({ className }: { className?: string }) {
    const canvasRef = useRef<HTMLCanvasElement>(null);

    useEffect(() => {
        let phi = 0;

        if (!canvasRef.current) return;

        const globe = createGlobe(canvasRef.current, {
            devicePixelRatio: 2,
            width: 600 * 2,
            height: 600 * 2,
            phi: 0,
            theta: 0.3,
            dark: 1,
            diffuse: 1.2,
            mapSamples: 20000,
            mapBrightness: 6,
            baseColor: [0.3, 0.3, 0.3], // Lighter gray for better visibility
            markerColor: [0.1, 0.8, 1], // Cyan markers
            glowColor: [0.1, 0.1, 0.2], // Slightly lighter glow
            markers: [
                { location: [37.7595, -122.4367], size: 0.05 }, // SF
                { location: [40.7128, -74.006], size: 0.04 }, // NYC
                { location: [51.5074, -0.1278], size: 0.04 }, // London
                { location: [1.3521, 103.8198], size: 0.04 }, // Singapore
                { location: [35.6762, 139.6503], size: 0.04 }, // Tokyo
                { location: [52.5200, 13.4050], size: 0.04 }, // Berlin
                { location: [-33.8688, 151.2093], size: 0.04 }, // Sydney
                { location: [25.2048, 55.2708], size: 0.04 }, // Dubai
            ],
            onRender: (state) => {
                // Called on every animation frame.
                // `state` will be an empty object, return updated params.
                state.phi = phi;
                phi += 0.005;
            },
        });

        return () => {
            globe.destroy();
        };
    }, []);

    return (
        <div className={`relative flex items-center justify-center w-full h-[600px] ${className}`}>
            <canvas
                ref={canvasRef}
                style={{ width: 600, height: 600, maxWidth: "100%", aspectRatio: 1 }}
                className="opacity-80 transition-opacity duration-1000 ease-in-out hover:opacity-100"
            />
        </div>
    );
}
