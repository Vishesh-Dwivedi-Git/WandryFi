...
export default function LeaderboardPage() {
  ...
  const { data: userProfit, isLoading: isLoadingUserProfit } = useReadContract({
    ...contract,
  ...
  const currentUserData: LeaderboardPlayer | null = address
    ? {
        rank: "?",
        address: `${address.slice(0, 6)}...${address.slice(-4)}`,
        score: userProfit ? parseFloat(formatEther(userProfit as bigint)) : 0,
        badge: "",
      }
    : null;
              ...
              >
                <div className="grid grid-cols-4 gap-4 items-center">
                  <div className="font-pixel text-lg">
                    <span
                      className={
                        typeof player.rank === "number" && player.rank <= 3
                          ? "text-neon-gold"
                          : "text-foreground"
                      }
                    >
          ...
          {address && (
            <div className="px-6 py-4 bg-neon-cyan/10 border-t-2 border-neon-cyan">
              {isLoadingUserProfit ? (
                ...
                currentUserData && (
                  <div className="grid grid-cols-4 gap-4 items-center">
                    <div className="font-pixel text-lg text-neon-cyan">
                    ...
                    <div className="font-pixel text-neon-cyan">
                      {currentUserData.score.toLocaleString()} WNDR
                    </div>
                    <div className="text-2xl">{currentUserData.badge}</div>
                  </div>
                )
              )}
            </div>
