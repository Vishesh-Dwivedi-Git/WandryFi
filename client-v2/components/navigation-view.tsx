...
import { X, Target, MapPin, Loader2, Trophy, Zap, Navigation, Star, Shield, Sword } from "lucide-react"
...
// ✅ HARDCORE SINGLE DESTINATION
const HARDCODED_DESTINATION = {
  id: "3",
  name: "🏛️ Hampi Ruins",
  lat: 15.3350,
  lng: 76.4600,
  difficulty: "Medium",
  xpReward: 750,
  description: "Uncover ancient empire secrets",
  stakeAmount: 100
}
  ...
  return L.divIcon({
    html: `
      <div class="relative">
        ...
        <div class="relative w-12 h-12 bg-cyan-400 rounded-full border-2 border-white shadow-lg flex items-center justify-center overflow-hidden" ${heading !== undefined ? `style="transform: rotate(${heading}deg)"` : ''}>
      ...
      watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          const bounds = L.latLngBounds([
            ...
            [HARDCODED_DESTINATION.lat, HARDCODED_DESTINATION.lng]
          ])
...
export default function NavigationView({ 
  ...
  useEffect(() => {
    if (userLocation) {
      const distance = calculateDistance(
        userLocation.lat,
        userLocation.lng,
        HARDCODED_DESTINATION.lat,
        HARDCODED_DESTINATION.lng
      )
  ...
  const handleCheckIn = async () => {
    ...
    setTimeout(() => {
      setIsCheckedIn(true)
      completeQuest(HARDCODED_DESTINATION.id)
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black font-mono">
        ...
        <div className="flex items-center justify-between px-4 h-full">
          <div className="flex items-center space-x-4">
            <h1 className="font-bold text-xl text-cyan-400">
              QUEST: {HARDCODED_DESTINATION.name}
            </h1>
            <Badge className="bg-orange-600 text-black font-bold px-3 py-1 text-xs">
              {HARDCODED_DESTINATION.difficulty.toUpperCase()}
            </Badge>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                <div className="bg-yellow-500 text-black px-3 py-1 rounded font-bold text-sm">
                  +{HARDCODED_DESTINATION.xpReward}XP
                </div>
        ...
        <div className="p-4">
          <div className="mb-6">
            ...
            <div className="bg-zinc-900 rounded border border-cyan-400 p-4">
              <h3 className="font-bold text-lg text-white mb-2">{HARDCODED_DESTINATION.name}</h3>
              <p className="text-sm text-white mb-3">{HARDCODED_DESTINATION.description}</p>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-yellow-500 font-bold">+{HARDCODED_DESTINATION.xpReward}XP</span>
                </div>
      ...
      <div className={`fixed left-80 ${userLocation ? 'top-40' : 'top-16'} right-0 bottom-0`}>
        {userLocation ? (
          <MapContainer 
          ...
          >
            ...
            {createDestinationIcon(distanceToTarget) && (
              <Marker
                position={[HARDCODED_DESTINATION.lat, HARDCODED_DESTINATION.lng]}
                icon={createDestinationIcon(distanceToTarget)!}
              >
                <Popup>
                  <div className="p-4 text-center font-mono">
                    <h3 className="text-cyan-400 font-bold text-lg mb-2">{HARDCODED_DESTINATION.name}</h3>
                    <p className="text-sm text-white mb-3">{HARDCODED_DESTINATION.description}</p>
                    <div className="flex items-center justify-center space-x-3 mb-3">
                      <Badge className="bg-orange-600 text-black font-bold text-xs">
                        {HARDCODED_DESTINATION.difficulty}
                      </Badge>
                    ...
                    <p className="text-cyan-400 font-bold">
                      Distance: {distanceToTarget < 1000 ? `${Math.round(distanceToTarget)}m` : `${(distanceToTarget/1000).toFixed(1)}km`}
                    </p>
                  </div>
                </Popup>
              </Marker>
            ...
            {/* Check-in zone */}
            <Circle
              center={[HARDCODED_DESTINATION.lat, HARDCODED_DESTINATION.lng]}
              radius={50}
              pathOptions={{
          ...
          <div className="flex items-center justify-center h-full bg-black">
            <div className="text-center">
              ...
              <p className="text-white text-lg font-bold mb-6">Activate GPS to begin</p>
              <Button
                onClick={requestLocation}
                disabled={isRequestingLocation}
                size="lg"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50">
          <Card className="bg-green-900 border-2 border-green-500 max-w-lg mx-4">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-green-300 font-bold text-lg mb-4">{HARDCODED_DESTINATION.name} Conquered!</p>
              
              <div className="bg-yellow-500 text-black px-4 py-2 rounded font-bold text-lg mb-4">
                +{HARDCODED_DESTINATION.xpReward}XP EARNED!
              </div>
