    ...
    Pausable
{
    ...
    uint256 public constant CLAIM_WINDOW = 1 days; // Users must claim within 1 day of travel date
    ...
    ) ERC721("Wanderify Journey", "WJRNY") Ownable() {
    ...
    function stake(
        ...
        if (
            !(commitments[msg.sender].isProcessed ||
                commitments[msg.sender].user == address(0))
        ) {
    ...
    function checkIn(
        ...
        if (block.timestamp < commitment.travelDate)
            revert TravelDateNotReached();
        if (block.timestamp > commitment.travelDate + CLAIM_WINDOW)
            revert ClaimWindowExpired();

        bytes32 messageHash = keccak256(
            abi.encodePacked(msg.sender, commitment.destinationId)
        );
        address signer = recoverSigner(messageHash, _signature);
        if (signer != verifierAddress) revert InvalidSignature();

        uint256 placeValueBonus = (BASE_REWARD *
            (100 + (BETA * placeValues[commitment.destinationId]))) / 100;
        uint256 poolCap = locationPools[commitment.destinationId] / 10;
        uint256 emission = (placeValueBonus < poolCap)
            ? placeValueBonus
            : poolCap;
        ...
        if (platformFeePercent > 0) {
            uint256 originalStake = (commitment.amountInPool * 100) /
    ...
    function processFailure() public {
        ...
        if (commitment.isProcessed) revert CommitmentAlreadyProcessed();
        if (block.timestamp <= commitment.travelDate)
            revert TravelDeadlineNotPassed();
    ...
    function getUserJourneyNFTs(
        address user
    ) public view returns (uint256[] memory) {
        return userJourneyNFTs[user];
    }

    function getJourneyNFTDetails(
        uint256 tokenId
    ) public view returns (JourneyNFT memory) {
    ...
    function setDestinationMetadata(
        uint256 _destinationId,
        string memory metadataURI
    ) public onlyOwner {
        destinationMetadata[_destinationId] = metadataURI;
    }
