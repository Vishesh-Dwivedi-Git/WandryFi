...
const destinations = {
    1: { lat: 15.3647, lon: 75.1239, country: "IN" },  // Example: Nrupatunga Betta, Hubballi
    2: { lat: 15.3822, lon: 74.9801, country: "IN" }   // Example: Unkal Lake Park, Hubballi
};
...
app.post('/api/verify', async (req, res) => {
    try {
        ...
        if (!walletAddress || !destinationId || userLat === undefined || userLon === undefined) {
        ...
        if (userIp && userIp !== '::1') {
            try {
                const ipInfo = await axios.get(`http://ip-api.com/json/${userIp}?fields=status,countryCode,proxy,hosting`);
                ...
                return res.status(500).json({ error: 'Location integrity verification failed.' });
            }
        }
        ...
        if (distance > 50) {
            return res.status(403).json({ error: `You are ${Math.round(distance)} meters away.` });
        }
        ...
        const signature = await verifierWallet.signMessage(ethers.utils.arrayify(messageHash));
